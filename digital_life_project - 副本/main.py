...
with block:
    ...
    with gr.Tab(label="TeaseFaces"):
        ...
        with gr.Tab(label="Inswapfaces"):
            with gr.Row():
                ...
                with gr.Column():
                    source_img = gr.Image(label="Original Photo", source="upload", type="pil")
                    target_img = gr.Image(label="Target Photo", source="upload", type="pil")
                    run_button = gr.Button(label="Run Face Swap")
                    with gr.Accordion("Options", open=True):
                        swap_face_no = gr.Number(label="Swap Face Number", value=1)
                with gr.Column():
                    result_image = gr.Image(label="Result Photo", source="canvas", type="pil")
            inputs = [
                source_img,
                target_img,
                swap_face_no
    ...
    with gr.Tab(label="CloneSounds"):
        ...
        with gr.Tab(label="Text-To-Speech"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    run_button = gr.Button(label="Convert to Speech")
                with gr.Column():
        ...
        with gr.Tab(label="Seperating"):
            with gr.Row():
                with gr.Column():
                    source_audio = gr.Audio(label="Original Audio", source="upload")
                    run_button = gr.Button(label="Run Sound Separating")
                    with gr.Accordion("Options", open=True):
